// Author: Watson
// Purpose: Benchmark research model throughput with deterministic input corpus.
...
bool WriteSyntheticCoeffFile(const std::filesystem::path& file_path) {
  ...
  out << "Synthetic DTM research coefficients for performance benchmark\n";
  out << "96\n";
...
std::vector<dtm2020::ResearchInputs> MakeCorpus() {
  std::vector<dtm2020::ResearchInputs> corpus;
  corpus.reserve(4096);
  for (int i = 0; i < 4096; ++i) {
    ...
    in.f30 = 70.0 + std::fmod(0.51 * x, 230.0);
    in.f30m = 70.0 + std::fmod(0.47 * x, 230.0);
    for (int k = 0; k < 10; ++k) {
      in.ap60[static_cast<std::size_t>(k)] = std::fmod(5.0 + 0.11 * x + 0.37 * static_cast<double>(k), 70.0);
    }
    corpus.push_back(in);
  }
  return corpus;
...
int main() {
  ...
  ns_per_eval.reserve(static_cast<std::size_t>(kSamples));
